...
import rospy
import rospkg
from pacman_msgs.srv import PacmanInitializationInfo
from pacman_msgs.msg import MapLayout
from pacman_msgs.srv import PacmanMapInfo
...
def readCommand( argv ):
    ...
    def getInitializationInfo(req):
        info = [options.layout, options.numGhosts]
        return info
    rospy.Service('pacman/inialize_game_state', PacmanInitializationInfo, getInitializationInfo)
    ...
    def getLayoutInfo(req):
        ...
        return info
    rospy.Service('pacman/initialize_map_layout', PacmanMapInfo, getLayoutInfo)
...
def loadAgent(pacman, nographics):
    ...
    pkg_path = rospack.get_path('pacman_game') + '/src'

    if not os.path.isdir(pkg_path):
        raise Exception('The package path: ' + pkg_path + ' is not valid.')
    moduleNames = [f for f in os.listdir(pkg_path) if f.endswith('gents.py')]
